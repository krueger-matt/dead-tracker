...
function AttendedStats({ shows, showData, selectedBand, onBandChange, availableBands }) {
  ...
  // Get all attended show IDs (filtered by selected band)
  const attendedShowIds = useMemo(() => {
    const bandShowIds = new Set(shows.map(show => show.id));
    return Object.keys(showData).filter(showId =>
      bandShowIds.has(showId) && showData[showId]?.attended
    );
  }, [showData, shows]);

  const attendedShowCount = attendedShowIds.length;
  ...
  useEffect(() => {
    const fetchSongStats = async () => {
      ...
      // Fetch all setlists for attended shows
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-gray-500">Analyzing shows you've attended</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Shows You've Attended</h1>
          ...
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <p className="text-gray-500 mb-6">
              You haven't marked any {selectedBand === 'all' ? '' : selectedBand + ' '}shows as attended yet! Use the purple circle (●) to mark shows you've been to in person.
            </p>
            <button
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            Based on {attendedShowCount} show{attendedShowCount !== 1 ? 's' : ''} you've been to in person
          </p>
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-white">
            <h2 className="text-xl font-semibold text-gray-900">
              Songs You've Heard Live
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              {songStats.length} unique songs across shows you attended
            </p>
          </div>

          <div className="divide-y divide-gray-100">
            {songStats.map((song, index) => {
              const percentage = Math.round((song.count / attendedShowCount) * 100);

              return (
                <div
                  key={song.songId}
                  onClick={() => handleSongClick(song)}
                  className="p-4 hover:bg-purple-100 transition-colors flex items-center justify-between cursor-pointer"
                >
                  <div className="flex items-center gap-4 flex-grow min-w-0">
                    ...
                    <div className="flex-grow min-w-0">
                      ...
                      <div className="text-xs text-gray-500">
                        Heard live in {percentage}% of shows you attended
                      </div>
                    </div>
                  </div>
        ...
        {/* Link to all listening stats */}
        <div className="mt-6 text-center">
          <button
            onClick={() => navigate('/stats')}
            className="text-blue-600 hover:text-blue-800 text-sm"
          >
            View all listening stats →
          </button>
        </div>
      </div>
          ...
          >
            ...
            <div className="p-6 border-b border-gray-200 bg-purple-50">
              ...
              <p className="text-sm text-gray-600 mt-1">
                Heard live in {selectedSong.count} show{selectedSong.count !== 1 ? 's' : ''} you attended
              </p>
            </div>
